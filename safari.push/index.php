...
$output_zip_file_name = ($package_version == 1) ? SIGNATURE_INITIALS.'v1'.'.zip' : SIGNATURE_INITIALS.'v2'.'.zip';
...
function raw_files() {
    return array(
        ...
        'icon.iconset/icon_64x64x.png',
        'icon.iconset/icon_64x64x@2x.png',
...
function create_signature($package_dir, $cert_path, $cert_password) {
	...
	$wwcrca_certificate_path = CERTIFICATES_FOLDER.DS.APPLE_WWCRCA_CERTIFICATE_PEM_FILENAME;

    // Sign the manifest.json file with the private key from the certificate
    $cert_data = openssl_x509_read($certs['cert']);
    $private_key = openssl_pkey_get_private($certs['pkey'], $cert_password);
    openssl_pkcs7_sign("$package_dir/manifest.json", $signature_path, $cert_data, $private_key, array(), PKCS7_BINARY | PKCS7_DETACHED, $wwcrca_certificate_path);
...
function create_push_package() {
	...
	$package_dir = _FILE_TMPPATH.'pushPackage' . time();
